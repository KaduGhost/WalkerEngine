...
public class InventarioPunirHMutes {
	...
	public InventarioPunirHMutes(int pagina, Player p) {
		...
		List<Punicao> punicoes = WalkerEngine.get().getPunicoesManager().getHistoricoPunicoes(PunicoesType.MUTE);
		...
		for (int i = 0; i < paginas; i++) {
			...
			for (int f = 0; f < 45; f++) {
				...
				inv.setItem(f, WalkersItens.getItemPunicao("punicaohistorico", j.getNickOriginal(), punicoes.get(i*45+f)));
			}
		}
