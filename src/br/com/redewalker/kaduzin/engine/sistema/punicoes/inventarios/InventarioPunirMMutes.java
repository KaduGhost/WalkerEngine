...
public class InventarioPunirMMutes {
	...
	public InventarioPunirMMutes(int pagina, Player p) {
		...
		List<Punicao> punicoes = WalkerEngine.get().getPunicoesManager().getMeuHistoricoPunicoes(PunicoesType.MUTE, j);
		...
		for (int i = 0; i < paginas; i++) {
			...
			for (int f = 0; f < 45; f++) {
				...
				inv.setItem(f, WalkersItens.getItemPunicao("punicaomeuhistorico", j.getNickOriginal(), punicoes.get(i*45+f)));
			}
		}
